...
class Helper
{
     ...
     *
     * $names = Helper::generateModuleNames($module_name);
     *
     ...
     *
     * Helper::parseValues($field['popup_vals']);
     *
            ...
            foreach($objects as $object) {
                if($object != "." && $object != "..") {
                    if(is_dir($dir . "/" . $object))
                        Helper::recurse_delete($dir . "/" . $object);
                    else
                        unlink($dir . "/" . $object);
                }
            }
     ...
     *
     * Helper::createThumbnail($filepath, $thumbpath, $thumbnail_width, $thumbnail_height);
     *
    ...
    public static function print_menu_editor($menu)
    {
        ...
        if(count($childrens) > 0) {
            ...
            foreach($childrens as $children) {
                $str .= Helper::print_menu_editor($children);
            }
            $str .= '</ol>';
        }
        $str .= '</li>';
    ...
    public static function print_menu($menu, $active = false)
    {
        ...
        $str = '<li' . $treeview . ' ' . $active_str . '><a href="' . url(config("penciladmin.adminRoute") . '/' . $menu->url) . '"><i class="fa ' . $menu->icon . '"></i> <span>' . Helper::real_module_name($menu->name) . '</span> ' . $subviewSign . '</a>';
        ...
        if(count($childrens)) {
            ...
            foreach($childrens as $children) {
                ...
                if(Module::hasAccess($module->id)) {
                    $str .= Helper::print_menu($children);
                }
            }
            $str .= '</ul>';
        }
    ...
    public static function print_menu_topnav($menu, $active = false)
    {
        ...
        $str = '<li ' . $treeview . '' . $active_str . '><a ' . $treeview2 . ' href="' . url(config("penciladmin.adminRoute") . '/' . $menu->url) . '">' . Helper::real_module_name($menu->name) . $subviewSign . '</a>';

        if(count($childrens)) {
            $str .= '<ul class="dropdown-menu" role="menu">';
            foreach($childrens as $children) {
                $str .= Helper::print_menu_topnav($children);
            }
            $str .= '</ul>';
        }
        $str .= '</li>';
    ...
    public static function setenv($param, $value)
    {
        ...
        $envfile = Helper::openFile('.env');
        $line = Helper::getLineWithString('.env', $param . '=');
     ...
     *
     * Helper::get_migration_file("students_table");
     *
