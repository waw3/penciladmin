...
namespace Waw3\PencilAdmin\Helpers;
...
/**
 * Class PAHelper
 * @package Waw3\PencilAdmin\Helpers
 *
 * This is PencilAdmin Helper class contains methods required for Admin Panel functionality.
 */
class PAHelper
{
    /**
     ...
     *
     * $names = PAHelper::generateModuleNames($module_name);
     *
    ...
    /**
     * Get list of Database tables excludingPencilAdmin Context tables like
     * backups, pa_configs, pa_menus, migrations, modules, module_fields, module_field_types
     ...
     *
     * PAHelper::parseValues($field['popup_vals']);
     *
     ...
     *
     * PAHelper::log("info", "", $commandObject);
     *
    ...
    public static function recurse_copy($src, $dst)
    {
        ...
        while(false !== ($file = readdir($dir))) {
            if(($file != '.') && ($file != '..')) {
                if(is_dir($src . '/' . $file)) {
                    PAHelper::recurse_copy($src . '/' . $file, $dst . '/' . $file);
                } else {
                    // ignore files
            ...
            foreach($objects as $object) {
                if($object != "." && $object != "..") {
                    if(is_dir($dir . "/" . $object))
                        PAHelper::recurse_delete($dir . "/" . $object);
                    else
                        unlink($dir . "/" . $object);
                }
            }
     ...
     *
     * PAHelper::createThumbnail($filepath, $thumbpath, $thumbnail_width, $thumbnail_height);
     *
     ...
     *
     * PAHelper::print_menu_editor($menu)
     *
    ...
    {
        $editing = \Collective\Html\FormFacade::open(['route' => [config('penciladmin.adminRoute') . '.pa_menus.destroy', $menu->id], 'method' => 'delete', 'style' => 'display:inline']);
        ...
        $childrens = \Waw3\PencilAdmin\Models\Menu::where("parent", $menu->id)->orderBy('hierarchy', 'asc')->get();

        if(count($childrens) > 0) {
            $str .= '<ol class="dd-list">';
            foreach($childrens as $children) {
                $str .= PAHelper::print_menu_editor($children);
            }
            $str .= '</ol>';
        }
        $str .= '</li>';
    ...
    public static function print_menu($menu, $active = false)
    {
        ...
        $str = '<li' . $treeview . ' ' . $active_str . '><a href="' . url(config("penciladmin.adminRoute") . '/' . $menu->url) . '"><i class="fa ' . $menu->icon . '"></i> <span>' . PAHelper::real_module_name($menu->name) . '</span> ' . $subviewSign . '</a>';
        ...
        if(count($childrens)) {
            ...
            foreach($childrens as $children) {
                ...
                if(Module::hasAccess($module->id)) {
                    $str .= PAHelper::print_menu($children);
                }
            }
            $str .= '</ul>';
        }
     ...
     * This needs to be done recursively
     *
     * PAHelper::print_menu_topnav($menu)
     *
     * @param $menu menu array from database
    ...
    public static function print_menu_topnav($menu, $active = false)
    {
        ...
        $str = '<li ' . $treeview . '' . $active_str . '><a ' . $treeview2 . ' href="' . url(config("penciladmin.adminRoute") . '/' . $menu->url) . '">' . PAHelper::real_module_name($menu->name) . $subviewSign . '</a>';

        if(count($childrens)) {
            $str .= '<ul class="dropdown-menu" role="menu">';
            foreach($childrens as $children) {
                $str .= PAHelper::print_menu_topnav($children);
            }
            $str .= '</ul>';
        }
        $str .= '</li>';
    ...
    public static function setenv($param, $value)
    {
        ...
        $envfile = PAHelper::openFile('.env');
        $line = PAHelper::getLineWithString('.env', $param . '=');
     ...
     *
     * PAHelper::get_migration_file("students_table");
     *
