...
class RolesController extends Controller
{
	...
	public function store(Request $request)
	{
		if(Module::hasAccess("Roles", "create")) {
			...
			return redirect()->route(config('penciladmin.adminRoute') . '.roles.index');
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($listing_cols); $j++) {
				...
				if($col == $module->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('penciladmin.adminRoute') . '/roles/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Roles", "edit")) {
					$output .= '<a href="'.url(config('penciladmin.adminRoute') . '/roles/'.$data->data[$i][0].'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}

				if(Module::hasAccess("Roles", "delete")) {
					$output .= Form::open(['route' => [config('penciladmin.adminRoute') . '.roles.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline']);
	...
	public function save_module_role_permissions(Request $request, $id)
	{
		if(Entrust::hasRole('SUPER_ADMIN')) {
			...
			return redirect(config('penciladmin.adminRoute') . '/roles/'.$id.'#tab-access');
		} else {
			return redirect(config('penciladmin.adminRoute')."/");
		}
	}
