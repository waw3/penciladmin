		...
		if(Module::hasAccess("Users", "view")) {
			...
			if(isset($user->id)) {
				if($user['type'] == "Employee") {
					return redirect(config('penciladmin.adminRoute') . '/employees/'.$user->id);
				} else if($user['type'] == "Client") {
					return redirect(config('penciladmin.adminRoute') . '/clients/'.$user->id);
				}
			} else {
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($listing_cols); $j++) {
				...
				if($col == $module->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('penciladmin.adminRoute') . '/users/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
