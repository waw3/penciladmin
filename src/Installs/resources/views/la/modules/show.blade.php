...
<div id="page-content" class="profile2">
	...
	@endif
		<div class="col-md-4">
			<div class="row">
				...
				<div class="col-md-9">
					<a class="text-white" href="{{ url(config('penciladmin.adminRoute') . '/'.$module->name_db) }}"><h4 data-toggle="tooltip" data-placement="left" title="Open {{ $module->model }} Module" class="name">{{ $module->label }}</h4></a>
					<div class="row stats">
	...
	<ul id="module-tabs" data-toggle="ajax-tab" class="nav nav-tabs profile" role="tablist">
		<li class=""><a href="{{ url(config('penciladmin.adminRoute') . '/modules') }}" data-toggle="tooltip" data-placement="right" title="Back to Modules"> <i class="fa fa-chevron-left"></i>&nbsp;</a></li>
	...
	<div class="tab-content">
						...
						<tbody>														
							@foreach ($module->fields as $field)
								<tr>
									...
									<td>
										<form id="listing_view_cal" action="{{ url(config('penciladmin.adminRoute') . '/module_field_listing_show') }}">
									...
									<td style="max-width:300px;"><?php echo PAHelper::parseValues($field['popup_vals']) ?></td>
									<td style="min-width:60px;">
										<a href="{{ url(config('penciladmin.adminRoute') . '/module_fields/'.$field['id'].'/edit') }}" class="btn btn-edit-field btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;" id="edit_{{ $field['colname'] }}"><i class="fa fa-edit"></i></a>
										<a href="{{ url(config('penciladmin.adminRoute') . '/module_fields/'.$field['id'].'/delete') }}" class="btn btn-edit-field btn-danger btn-xs" style="display:inline;padding:2px 5px 3px 5px;" id="delete_{{ $field['colname'] }}"><i class="fa fa-trash"></i></a>
										@if($field['colname'] != $module->view_col)
											<a href="{{ url(config('penciladmin.adminRoute') . '/modules/'.$module->id.'/set_view_col/'.$field['colname']) }}" class="btn btn-edit-field btn-success btn-xs" style="display:inline;padding:2px 5px 3px 5px;" id="view_col_{{ $field['colname'] }}"><i class="fa fa-eye"></i></a>
										@endif
									</td>
								</tr>
							@endforeach
		...
		<div role="tabpanel" class="tab-pane fade in p20 bg-white" id="tab-access">
			...
			<form action="{{ url(config('penciladmin.adminRoute') . '/save_role_module_permissions/'.$module->id) }}" method="post">
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				{{ Form::open(['route' => [config('penciladmin.adminRoute') . '.modules.destroy', 0], 'id' => 'module_del_form', 'method' => 'delete', 'style'=>'display:inline']) }}
...
<div class="modal fade" id="AddFieldModal" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			{!! Form::open(['route' => config('penciladmin.adminRoute') . '.module_fields.store', 'id' => 'field-form']) !!}
...
<link rel="stylesheet" type="text/css" href="{{ asset('pa-assets/plugins/bootstrap-slider/slider.css') }}"/>
<style>
...
<script src="{{ asset('pa-assets/plugins/bootstrap-slider/bootstrap-slider.js') }}"></script>
<script src="{{ asset('pa-assets/plugins/jQueryUI/jquery-ui.js') }}"></script>
...
$(function () {
	...
	$('.delete_module').on("click", function () {
		...
		$.ajax({
			url: "{{ url(config('penciladmin.adminRoute') . '/get_module_files/') }}/" + module_id,
			type:"POST",
			beforeSend: function() {
	...
	$("#sortable_module_fields").sortable({
		update: function(event, ui) {
			...
			$.ajax({
				url: "{{ url(config('penciladmin.adminRoute') . '/save_module_field_sort') }}/"+{{ $module->id }},
	...
	$("#generate_migr").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url(config('penciladmin.adminRoute') . '/module_generate_migr') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
	...
	$("#generate_update").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url(config('penciladmin.adminRoute') . '/module_generate_update') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
				$fa.removeClass("fa-refresh");
				$fa.removeClass("fa-spin");
	...
	$("#generate_migr_crud").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url(config('penciladmin.adminRoute') . '/module_generate_migr_crud') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
				$fa.removeClass("fa-refresh");
				$fa.removeClass("fa-spin");
	...
	$('.Switch.Ajax').click(function() {
		...
		$.ajax({
			type: "POST",
			url : "{{ url(config('penciladmin.adminRoute') . '/module_field_listing_show') }}",
			data : {
				_token: '{{ csrf_token() }}',
