				...
				<div class="nav-tabs-custom">
					...
					<div class="tab-content">
						...
						<div class="tab-pane" id="tab-custom-link">
							...
							{!! Form::open(['action' => '\Waw3\PencilAdmin\Controllers\MenuController@store', 'id' => 'menu-custom-form']) !!}
				...
				<div class="dd" id="menu-nestable">
					<ol class="dd-list">
						@foreach ($menus as $menu)
							<?php echo PAHelper::print_menu_editor($menu); ?>
						@endforeach
					</ol>
				</div>
			</div>
...
<div class="modal fade" id="EditModal" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			{!! Form::open(['action' => ['\Waw3\PencilAdmin\Controllers\MenuController@update', 1], 'id' => 'menu-edit-form']) !!}
...
@push('scripts')
<script src="{{ asset('pa-assets/plugins/nestable/jquery.nestable.js') }}"></script>
...
$(function () {
	...
	$('#menu-nestable').on('change', function() {
		...
		$.ajax({
			url: "{{ url(config('penciladmin.adminRoute') . '/pa_menus/update_hierarchy') }}",
			method: 'POST',
			data: {
				jsonData: jsonData,
				"_token": '{{ csrf_token() }}'
	...
	$("#tab-modules .addModuleMenu").on("click", function() {
		...
		$.ajax({
			url: "{{ url(config('penciladmin.adminRoute') . '/pa_menus') }}",
			method: 'POST',
			data: {
				type: 'module',
				module_id: module_id,
