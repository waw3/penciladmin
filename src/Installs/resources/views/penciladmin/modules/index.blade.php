	...
	<div class="box-body">
		...
		<tbody>
			...
			@foreach ($modules as $module)
				<tr>
					<td>{{ $module->id }}</td>
					<td><a href="{{ url(config('penciladmin.adminRoute') . '/modules/'.$module->id) }}">{{ $module->label }}</a></td>
					...
					<td>
						...
						<a href="{{ url(config('penciladmin.adminRoute') . '/modules/'.$module->id)}}#access" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-key"></i></a>
						<a href="{{ url(config('penciladmin.adminRoute') . '/modules/'.$module->id)}}#sort" class="btn btn-success btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-sort"></i></a>
...
<div class="modal fade" id="AddExistModal" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			{!! Form::open(['route' => config('penciladmin.adminRoute') . '.modules.store', 'id' => 'module-add-form']) !!}
...
<script src="{{ asset('pa-assets/plugins/iconpicker/fontawesome-iconpicker.js') }}"></script>
<script>
$(function () {
	...
	$('.update_module').on("click", function () {
		...
		$('.save_edit_module').on("click", function () {
			...
			$.ajax({
				url: "{{ url(config('penciladmin.adminRoute') . '/module_update') }}",
				type:"POST",
