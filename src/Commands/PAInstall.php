 ...
 *
 * Command to install PencilAdmin package into project which moves lot of file from 'src/Installs' directory to Project
 */
class PAInstall extends Command
{
    ...
    protected $signature = 'pa:install';

    // The command description.
    protected $description = 'Install PencilAdmin Package. Generate whole structure for /admin.';
    ...
    /**
     * Generates and Moves files to install PencilAdmin package.
    ...
    public function handle()
    {
        try {
            ...
            if($this->confirm("Want to set your Database config in the .env file ?", true)) {
                ...
                $db_data['db'] = $this->ask('Database Name', 'penciladmin1');
            ...
            if($this->confirm("This process may change/append to the following of your existing project files:"
                ...
                */
                ...
                // pa-assets
                $this->line('Generating PencilAdmin Public Assets...');
                $this->replaceFolder($from . "/pa-assets", $to . "/public/pa-assets");
                ...
                if($driver_type != "array") {
                    throw new Exception("Please set Cache Driver to array in .env (Required for Zizaco\Entrust) and run pa:install again:"
                        . "\n\n\tCACHE_DRIVER=array\n\n", 1);
                }
                ...
                // $this->call('db:seed', ['--class' => 'PencilAdminSeeder']);
                ...
                $this->info("\nPencilAdmin successfully installed.");
                $this->info("You can now login from yourdomain.com/" . config('penciladmin.adminRoute') . " !!!\n");
            ...
            if(strpos($msg, 'SQLSTATE') !== false) {
                throw new Exception("PAInstall: Database is not connected. Connect database (.env) and run 'pa:install' again.\n" . $msg, 1);
            } else {
                $this->error("PAInstall::handle exception: " . $e);
                throw new Exception("PAInstall::handle Unable to install : " . $msg, 1);
            }
        }
    }
